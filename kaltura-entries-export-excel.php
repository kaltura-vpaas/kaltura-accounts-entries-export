...
class KalturaContentAnalytics implements ILogger 
{
	...
	const ENTRY_TYPE_IN = array(MediaType::VIDEO, MediaType::IMAGE, MediaType::AUDIO); //defines the entry types to retrieve 
	//const ENTRY_FIELDS = array('name', 'userId', 'msDuration', 'createdAt', 'updatedAt', 'lastPlayedAt', 'status', 'views', 'plays', 'tags', 'status', 'type', 'mediaType', 'referenceId', 'partnerId'); //the list of entry fields to export (only base metadata, no custom fields here), entryId, captions and categories will be added to this list
	const ENTRY_FIELDS = array('referenceId', 'name', 'userId', 'msDuration', 'createdAt', 'updatedAt', 'tags', 'status', 'mediaType');

	const PARENT_CATEGORIES = null;//186687053; // The IDs of the Kaltura Categories you'd like to export, set to `null` to export all.
	...
	const METADATA_PROFILE_ID = 13899113; // The profile id of the custom metadata profile to get its fields per entry
	...
	const REPORT_TIMEZONE_OFFSET = -180; //timezone offset for getting usage storage report for, negative 180 is Israel timezone
	...
	const LIMIT_TOTAL_ENTRIES = false; //, if false or null will be ignored
	...
	private function clearConsoleLines($last_lines) 
	{
	...
	public function run($pid, $secret) 
	{
		...
		foreach ($allSubAccounts as $subpartner) {
			...
			$this->clearConsoleLines(7);
			if (KalturaContentAnalytics::LIMIT_TOTAL_ENTRIES && count($this->allEntries) > KalturaContentAnalytics::LIMIT_TOTAL_ENTRIES) break;
		}
	...
	private function getEntriesForSubAccount($partnerId, $partnerName, $apiSecret)
	{
		...
		echo PHP_EOL . 'Total entries to export: ' . count($entries);
		...
		if (KalturaContentAnalytics::GET_CATEGORIES) {
			...
			for ($i = 0; $i < $N ; $i++) {
				...
				if (($i % KalturaContentAnalytics::CYCLE_SIZES == 0) || ($i == $N-1)) {
					...
					$catents = $this->getFullListOfKalturaObject($catfilter, $this->client->getCategoryEntryService(), 'categoryId', 'entryId*', false, 'Category');
			...
			for ($i = 0; $i < $N ; $i++) {
				...
				if (($i % KalturaContentAnalytics::CYCLE_SIZES == 0) || ($i == $N-1)) {
					...
					$catnames = $this->getFullListOfKalturaObject($catfilter, $this->client->getCategoryService(), 'id', ['name', 'fullName'], false, 'Category');
	...
	public function getFullListOfKalturaObject ($filter, $listService, $idField = 'id', $valueFields = NULL, $printProgress = FALSE, $stopOnCreatedAtDate = false, $objectName = null) {
		...
		echo PHP_EOL.'count: '. $count .' ===total count -  '.count($allObjects).PHP_EOL;
